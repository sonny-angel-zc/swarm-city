...
export default function ActivityFeed() {
  ...
  return (
    <div className="h-28 bg-[var(--bg-panel)] border-t border-[var(--border-subtle)] text-[var(--text-primary)] flex items-stretch overflow-hidden">
      {/* Activity Log */}
      <div className="flex-1 p-3 overflow-y-auto">
        <div className="flex items-center gap-2 mb-1.5">
          ...
          <span className="text-[10px] text-[var(--text-secondary)]">({log.length})</span>
        </div>
        {log.length === 0 ? (
          ...
          <div className="space-y-0.5">
            {log.slice(-12).reverse().map((entry, i) => (
              <div key={i} className="text-[11px] text-[var(--text-secondary)] leading-relaxed">
      ...
      <div className="w-48 border-l border-[var(--border-subtle)] p-3 flex flex-col justify-center">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-[11px] font-mono ${autonomous.enabled ? 'text-emerald-400' : 'text-[var(--text-secondary)]'}`}>
