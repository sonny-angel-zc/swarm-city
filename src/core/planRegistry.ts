...
const DOCS_SEED: PlanDocument[] = [
  ...
  },
  {
    ...
    title: 'Infrastructure Rules and Map Constraints (SWA-76 Subtask 1/9)',
    path: 'docs/testing/infrastructure-connections-constraints-swa-76-subtask-1.md',
    category: 'reference',
    track: 'foundation',
    updatedAt: '2026-03-03',
    summary: 'Authoritative pathing rules, tile types, waterway crossing rules, bridge placement conditions, and animation performance limits for roads, rail, and power grid connections between the central government district and the four outer district quadrants on the 24x24 grid.',
    tags: ['infrastructure', 'roads', 'rail', 'power-grid', 'bridges', 'districts', 'city-canvas', 'pathfinding', 'swa-76', 'constraints', 'research'],
    content: [
      'SWA-76 subtask 1/9 documents authoritative infrastructure rules for the 24x24 isometric city grid.',
      '- Road network: mainRows/mainCols [11,12] (center cross), outerRows/outerCols [4,5,18,19] (district boundary ring).',
      '- District quadrants: nw (0-7,0-7), ne (16-23,0-7), sw (0-7,16-23), se (16-23,16-23); 8x8 tiles each.',
      '- Waterway rule: water is perimeter-only (row 0, row 23, col 0, col 23); no interior waterways; all district-to-district paths are water-free.',
      '- Bridge condition: bridge tiles only at intersection of road cols/rows [4,5,11,12,18,19] with water perimeter tiles.',
      '- Pathfinding: 4-cardinal BFS, blocked by BUILDING_FOOTPRINT_TILES, connectors precomputed at module load.',
      '- Performance: 16.67ms frame budget, max 220 entities, route geometry precomputed — no BFS or gridToScreen in animation frame.',
      '- Identifies three implementation gaps: no arterial visual distinction, no rail rendering, no district power mesh.',
    ].join('\n'),
  },
  {
  ...
  {
    id: 'central-district-audit-swa-78-subtask-1',
    title: 'Central District Visual Audit (SWA-78 Subtask 1/10)',
    path: 'docs/testing/central-district-audit-swa-78-subtask-1.md',
    category: 'reference',
    track: 'foundation',
    updatedAt: '2026-03-03',
    summary: 'Code-verified audit of the central government district layout: fountain/plaza spatial mismatch, deco collision at Reviewer, empty civic approach corridor, missing civic zone definition, and asset gaps — with a coordinate-mapped gap list and ordered implementation guidance for SWA-78 subtasks 2–10.',
    tags: ['city', 'central-district', 'government-plaza', 'landmark', 'isometric', 'visual-design', 'audit', 'swa-78', 'research'],
    content: [
      'SWA-78 subtask 1/10 audits the current central district before any visual overhaul implementation.',
      '- Identifies critical fountain/plaza spatial mismatch: fountain drawn at road intersection (7.5,7.5) instead of City Hall plaza (11–12, 11–12).',
      '- Confirms deco-building collision at (9,6): NATURE_SPRITES entry overlaid on Reviewer/Courthouse footprint.',
      '- Documents 15 coordinate-mapped gaps covering empty tiles, missing civic zone, height hierarchy inversion, and sprite semantic mismatches.',
      '- Defines hard constraints (no drawBuilding changes, no runtime tile mutation, 60fps budget) and ordered subtask guidance for fountain relocation, civic zone, plaza ground differentiation, approach corridor, and landmark hierarchy.',
    ].join('\n'),
  },
  {
  ...
  {
    id: 'district-visual-direction-swa-75-subtask-1',
    title: 'District Visual Direction and References (SWA-75 Subtask 1/9)',
    path: 'docs/testing/district-visual-direction-swa-75-subtask-1.md',
    category: 'reference',
    track: 'foundation',
    updatedAt: '2026-03-03',
    summary: 'Establishes distinct art directions for Core Architecture, Dashboard & UX, Observability, and Self-Improvement districts with unique palette families, material sets, architectural motifs, and semantic color tokens in both light and dark modes.',
    tags: ['districts', 'theming', 'visual-design', 'art-direction', 'tokens', 'city-canvas', 'swa-75'],
    content: [
      'SWA-75 subtask 1/9 defines the visual art direction for all four quadrant districts.',
      '- Codified in src/core/districtTheme.ts as DISTRICT_ART_DIRECTIONS and per-district token tables.',
      '- NW Core Architecture: blue-steel palette, industrial glass and structural steel, grid-column motif.',
      '- NE Dashboard & UX: violet-lavender palette, translucent glass panels, curved facade motif.',
      '- SW Observability: emerald-teal palette, sensor-mesh cladding, antenna tower array motif.',
      '- SE Self-Improvement: amber-copper palette, oxidized metal and living materials, spiraling growth motif.',
      '- All tokens defined in both light and dark modes with low-alpha tints to preserve terrain legibility.',
      '- resolveDistrictThemeTokens() accepts optional projectColor override for borderStroke and taskBuildingBase only.',
    ].join('\n'),
  },
  {
