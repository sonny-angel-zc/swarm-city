...
- `npm run typecheck`: offline-safe TypeScript validation (`validate-next-router-roots` + `tsc --noEmit`).
- `TYPECHECK_RUN_NEXT_TYPEGEN=1 npm run typecheck`: additionally run optional `next typegen` before `tsc`.
